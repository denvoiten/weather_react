...
public class WeatherController {
    ...
    @GetMapping(value = "/cityGreaterThen/{temp}", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<Weather> findGreaterThen(@PathVariable Integer temp) {
        Flux<Weather> data = weathers.findTempGreaterThen(temp);
        Flux<Long> delay = Flux.interval(Duration.ofSeconds(3));
        return Flux.zip(data, delay).map(Tuple2::getT1);
    }
}
